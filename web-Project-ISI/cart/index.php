...
<body class="d-flex flex-column min-vh-100">
    ...
    <div class="container">
        <div class="row pt-4">
            <div
                class="col-lg-<?php if (count($_SESSION["shopping_cart"])) echo "8"; else echo "12";?> panier-contenu mx-1 bg-light">
                ...
                <div class="prod">
                    <div class="row my-2">
                        <div class="col-md-4 center">
                            <img src="../pictures/<?php echo $row["picture"];?>" height="220px" width="100%"
                                style="max-width: 300px;">
                        </div>
                        <div class="col-md-8">
                            <input type="text" class="d-none item_id" name="title"
                                value="<?php echo $item["item_id"]?>">
                            <input type="text" class="d-none item_av_quantity" name="item_av_quantity"
                                value="<?php echo $item["item_av_quantity"]?>">
                            <input type="text" class="d-none item_price" name="title"
                                value="<?php echo $item["item_price"]?>">

                            <h1 class="price_tag" id="<?php echo $item["item_id"]."-price" ?>">
                                <?php echo number_format($item["item_price"]*$item["item_quantity"],2)?>$</h1>
                            ...
                            <div class="d-flex align-items-center justify-content-start flex-st my-4">
                                ...
                                <div class="col-4 d-flex mr-3" style="height:40px">
                                    <button class="btn btn-dark" onclick="sous(this.id)"
                                        id="<?php echo $item["item_id"]."-sous" ?>">-</button>
                                    <div class="form-group">
                                        <input type="text" name="item_quantity"
                                            class="form-control quantity_btn item_quantity" style="width:50px"
                                            value="<?php echo $item["item_quantity"]?>" width=20
                                            id="<?php echo $item["item_id"] ?>" name="quantity">
                                    </div>
                                    <button class="btn btn-dark" onclick="add(this.id)"
                                        id="<?php echo $item["item_id"]."-add" ?>">+</button>
                                </div>
                            </div>
                            <button class="btn btn-danger remove_btn" id="<?php echo $item["item_id"]."-remove";?>"
                                class="remove" onclick="remove(this.id)" name="remove_btn">Remove</button>
                        </div>
                    </div>
                    <hr>
                </div>
    ...
    <script>
        ...
        const updateTotalPrice = () => {
            ...
            final_price.innerText = "Final price : " + Math.round(parseInt(total_price.innerText) * (1 + (parseInt(
                tax.innerText) / 100)) * 100) / 100 + "$"
        }
